...
void loop() {
  ...
  if (rx_str != "") {
    ...
    //if first item in rx_values array is equal to "motor" use the second item to set the motor speed
    if (strcmp(rx_values ,"motor") == 0){
    ...
    //if first item in rx_values array is equal to "fire" use the second item to set the motor speed and the third item to set the base position, then load and release ball
    if (strcmp(rx_values ,"goto") == 0){
      rx_int = atoi(strtok (NULL,","));
      goTo(rx_int);
      delay(1000); 
